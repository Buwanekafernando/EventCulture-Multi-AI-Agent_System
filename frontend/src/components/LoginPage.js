import React from 'react';
import { useAuth } from '../contexts/AuthContext';
import '../styles/login.css';

const LoginPage = () => {
  const { login } = useAuth();

  const handleGoogleLogin = () => {
    login();
  };

  return (
    <div className="app">
      {/* Header */}
      <header className="header">
        <div className="nav-left">
          <span className="logo">🎭 EventCulture</span>
        </div>
      </header>

      {/* Login Section */}
      <section className="login-section">
        <div className="login-container">
          <div className="login-card">
            <div className="logo-section">
              <div className="logo-icon">🎭</div>
              <h1>EventCulture</h1>
            </div>
            
            <div className="welcome-section">
              <h2>Welcome to EventCulture</h2>
              <p>Sign in to discover and manage amazing events across Sri Lanka</p>
            </div>

            <div className="login-form">
              <button className="google-login-btn" onClick={handleGoogleLogin}>
                <div className="google-icon">
                  <svg width="20" height="20" viewBox="0 0 24 24">
                    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                    <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                    <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                  </svg>
                </div>
                <span>Continue with Google</span>
              </button>
            </div>

            <div className="login-info">
              <h3>Why Sign In?</h3>
              <ul>
                <li>🎯 Get personalized event recommendations</li>
                <li>📍 Find events near you with interactive maps</li>
                <li>📊 Track your event history and preferences</li>
                <li>🎫 Easy booking and ticket management</li>
                <li>📈 Analytics dashboard (for organizers)</li>
              </ul>
            </div>

            <div className="role-info">
              <h4>Choose Your Role</h4>
              <div className="role-cards">
                <div className="role-card">
                  <div className="role-icon">👤</div>
                  <h5>Event Attendee</h5>
                  <p>Discover and book events that match your interests</p>
                </div>
                <div className="role-card">
                  <div className="role-icon">🎪</div>
                  <h5>Event Organizer</h5>
                  <p>Manage events and view analytics dashboard</p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* Footer */}
      <footer className="login-footer">
        <p>&copy; 2024 EventCulture. All rights reserved.</p>
      </footer>
    </div>
  );
};

export default LoginPage;
